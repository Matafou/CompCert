...
(* Processing of a .mlo file generated by sireum + aprk2Cminor +
   extraction + output_value. *)

let process_sparkobj_file sourcename =
  if !option_S then begin
    compile_sparkobj_file sourcename (output_filename ~final:true sourcename ".mlo" ".s");
    ""
  end else begin
    let asmname =
      if !option_dasm
      then output_filename sourcename ".mlo" ".s"
      else Filename.temp_file "compcert" ".s" in
    compile_sparkobj_file sourcename asmname;
    let objname = output_filename ~final: !option_c sourcename ".c" ".o" in
    assemble asmname objname;
    if not !option_dasm then safe_remove asmname;
    objname
...
(* File arguments *)
  ...
  Suffix ".S", Self (fun s ->
      push_action process_S_file s; incr num_source_files);
  Suffix ".mlo", Self (fun s ->
      push_action process_sparkobj_file s; incr num_source_files);
